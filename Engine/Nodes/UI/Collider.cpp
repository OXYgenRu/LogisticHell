...
UI::Collider::create(const std::shared_ptr<Node> &parent, const std::string &node_id, int render_priority) {
    auto node = std::make_shared<UI::Collider>(parent, node_id, render_priority);
...
void UI::Collider::handle_key_press(sf::Event &event, EngineContext &ctx, const sf::Vector2f &local_position) {
    this->on_key_press(event, ctx, local_position);
}

void UI::Collider::handle_key_release(sf::Event &event, EngineContext &ctx, const sf::Vector2f &local_position) {
    this->on_key_release(event, ctx, local_position);
}
...
void
UI::Collider::bind_on_key_press(
        const std::function<void(sf::Event &, EngineContext &, const sf::Vector2f &)> &new_function) {
    this->on_mouse_release = new_function;
}

void UI::Collider::bind_on_key_release(
        const std::function<void(sf::Event &, EngineContext &, const sf::Vector2f &)> &new_function) {
    this->on_key_release = new_function;
}
