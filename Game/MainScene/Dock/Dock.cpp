...
std::shared_ptr<Dock>
Dock::create(const std::shared_ptr<Node> &parent, EngineContext &ctx, const std::shared_ptr<GameWorld> &world,
    ...
    Dock::setup(node, ctx, world, world_camera, position, grid_size, b2_cell_size);
    parent->add_node(node);
    return node;
}


void Dock::setup(const std::shared_ptr<Dock> &node, EngineContext &ctx, const std::shared_ptr<GameWorld> &world,
                 const std::shared_ptr<CameraNode> &world_camera,
                 sf::Vector2f position,
                 sf::Vector2i grid_size, float b2_cell_size) {
    ...
    node->building_grid = BuildingGrid::create(node->camera, ctx, world->get_pixel_per_meter() * b2_cell_size,
                                               grid_size,
                                               "BuildingGrid");
    ...
    node->editor_controller = EditorController::create(ctx, node, node->building_grid, world);

    node->interface = EditorInterface::create(node, ctx, node, world, "DockInterface", 9);
...
void DockSpawner::setup(const std::shared_ptr<DockSpawner> &node, EngineContext &ctx,
    ...
    float sf_block_side_size = b2_block_side_size * scene->world->get_pixel_per_meter();
    ...
    node->dock_area->bind_on_mouse_release([weak_spawner, weak_scene](sf::Event &event, EngineContext &ctx) {
        ...
        scene->world->get_docks_system()->spawn_dock(ctx, spawner->position, spawner->grid_size,
                                                     spawner->b2_block_side_size,
                                                     nullptr);
    });
}
