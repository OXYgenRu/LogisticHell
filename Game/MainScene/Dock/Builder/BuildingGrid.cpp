...
BuildingGrid::create(const std::shared_ptr<Node> &parent, EngineContext &ctx, float sf_cell_size,
                     const sf::Vector2i &grid_size, const std::string &node_id, int render_priority) {
    auto node = std::make_shared<BuildingGrid>(parent, node_id, render_priority);
...
void BuildingGrid::setup(const std::shared_ptr<BuildingGrid> &node, float sf_cell_size,
    ...
    node->grid_collider = UI::Collider::create(node, "GridCollider");
    ...
    for (int i = 0; i < grid_size.y; i++) {
        for (int j = 0; j < grid_size.x; j++) {
            node->render_features[i][j] = Node::create(node, "RenderFeaturesAnchorBlock {x:" +
                                                             std::to_string(j) + ",y:" + std::to_string(i) +
                                                             "}");
            node->render_features[i][j]->set_position(
                    {float(j) * sf_cell_size, float(grid_size.y - i - 1) * sf_cell_size});

            node->preview_render_features[i][j] = Node::create(node, "PreviewRenderFeaturesAnchorBlock {x:" +
                                                                     std::to_string(j) + ",y:" + std::to_string(i) +
                                                                     "}", 1);
            node->preview_render_features[i][j]->set_position(
                    {float(j) * sf_cell_size, float(grid_size.y - i - 1) * sf_cell_size});

            node->mask[i][j] = TexturedQuad::create(node, "MaskAnchorBlock {x:" +
                                                          std::to_string(j) + ",y:" + std::to_string(i) +
                                                          "}", 2);
...
void BuildingGrid::add_render_feature(const UnitRenderFeature &new_feature,
    ...
    auto new_quad = TexturedQuad::create(
            render_features[new_feature.anchor_block.y][new_feature.anchor_block.x], "RenderFeature {x:" +
                                                                                     std::to_string(
                                                                                             new_feature.anchor_block.x) +
                                                                                     ",y:" + std::to_string(
                    new_feature.anchor_block.y) +
    ...
    for (int i = 0; i < 4; i++) {
        new_quad->quad[i].position = {new_feature.vertices[i].x * sf_cell_size,
                                      -new_feature.vertices[i].y * sf_cell_size};
    }
